    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try {
                ...
                String sqlCheck = "select * from person where person_id = '" + username + "' and password = '" + password + "'";
                ResultSet rs = stmt.executeQuery(sqlCheck);
                if (!rs.next()) {
                    response.getOutputStream().println("pop_up_modal");
                } else {
                    // Create Var keep type
                    String person_type = rs.getString("person_type");
                    // Create Obj Login
                    Login account = new Login(username, password, person_type);
                    // Cretate HTTP Session
                    HttpSession session = request.getSession();
                    // Set Attribute to Session
                    session.setAttribute("Account", account);
                    // Create RequestDispatcher and forward it.
