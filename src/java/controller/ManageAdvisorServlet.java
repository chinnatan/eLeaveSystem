 ...
 * @author kornc
 */
@WebServlet(name = "ManageAdvisorServlet", urlPatterns = {"/manageadvisor.do"})
public class ManageAdvisorServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (flag.equals("ack")) {
                String sqlUpdate = "update leavedocument "
                        + "set advisor = '" + professorName + "'"
                        + "where leave_id = '" + leaveid + "'";
                try {
                    stmt = conn.createStatement();
                    checkSum = stmt.executeUpdate(sqlUpdate);
                    if (checkSum > 0) {
                        response.sendRedirect("manageadvisorsuccess.html");
                    } else {
                        response.sendRedirect("manageadvisorfail.html");
                    }
